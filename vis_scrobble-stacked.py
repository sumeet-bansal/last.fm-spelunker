...
sql = 'SELECT {0}, date_uts, count({0}) FROM scrobbles GROUP BY {0}, date_year, date_month, date_date HAVING count({0}) > 5'.format(metric)
result = db.query(sql)
...
for row in result:
	...
	if artist not in streams:
		streams[artist] = [0 for i in range(366)]
	else:
		dt = datetime.fromtimestamp(int(row['date_uts']))
		streams[artist][int(dt.strftime('%j'))] = row['count(%s)' % metric]
...
for artist, plays in streams.items():
	chart.add_serie(name=artist, y=plays, x=[i for i in range(366)])
chart.buildhtml()
